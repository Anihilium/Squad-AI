...
{
    AIAgent healerAgent;
    public Agent healTarget;
    List<Agent> allyAgents = new List<Agent>();
    ...
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        healerAgent = animator.GetComponent<AIAgent>();

        Transform allies = GameObject.Find("Allies").transform;

        for (int i = 0; i < allies.childCount; ++i)
        {
            Agent curAgent = allies.GetChild(i).GetComponent<Agent>();

            if (curAgent != healerAgent)
                allyAgents.Add(curAgent);
        }
    }
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        {
            healerAgent.MoveTo(healTarget.transform.position);
            return;
        }
